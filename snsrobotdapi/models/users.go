...
type Users struct {
	Id     int       `orm:"column(u_id);pk"`
	UName  string    `orm:"column(u_name)"`
	UPass  string    `orm:"column(u_pass)"`
	UGroup string    `orm:"column(u_group)"`
	URank  float64   `orm:"column(u_rank)"`
	UDate  time.Time `orm:"column(u_date);type(timestamp without time zone);null"`
	UIpv4  string    `orm:"column(u_ipv4);null"`
}
