...
type Users struct {
	Id       int       `orm:"column(_id);pk"`
	Username string    `orm:"column(_username);null"`
	Password string    `orm:"column(_password);null"`
	Group    string    `orm:"column(_group);null"`
	Date     time.Time `orm:"column(_date);type(timestamp without time zone);null"`
	Ipv4     string    `orm:"column(_ipv4);null"`
	Rank     float64   `orm:"column(_rank);null"`
}
